...
- category: категория товара
##### Методы:
- getDetails(): { id: string, name: string, price: number, imageUrl: string}: Возвращает полную информацию о товаре в виде объекта.
#### Класс ContactInfo
##### Конструктор:
...
### Слой Презентера
Презентер управляет логикой взаимодействия между моделью и представлением, обрабатывая события и вызывая соответствующие методы модели. Основная задача презентера — обработка событий, исходящих от представления, обновление модели на основе действий пользователя и обновление представления на основе изменений в модели.
##### Основные задачи презентера:
- Управление данными корзины (добавление, удаление товаров).
- Запуск процесса оформления заказа (например, отображение модального окна).
- Слушать изменения в модели и обновлять представление.
- Обрабатывать действия пользователя (добавление товаров в корзину, изменение способа оплаты).